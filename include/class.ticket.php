...
implements RestrictedAccess, Threadable, Searchable {
    ...
    static protected $perms = array(
            ...
            self::PERM_RESOLVE => array(
                'title' =>
                /* @trans */ 'Resolve',
                'desc'  =>
                /* @trans */ 'Ability to resolve tickets'),
    ...
    function setStatus($status, $comments='', &$errors=array(), $set_closing_agent=true, $force_close=false) {
        ...
        if ($role && $this->getStatusId()) {
            ...
            // Solo pueden resolver tickets abiertos, no modificar tickets ya resueltos
            if ($role->hasPerm(Ticket::PERM_RESOLVE) && !$role->hasPerm(Ticket::PERM_CLOSE)) {
                ...
                if ($currentStatus && $currentStatus->getId() == 2) { // Ticket ya está "Resolved"
                    return false; // No puede cambiar el estado de un ticket ya resuelto
                }
            }
            
            switch ($status->getState()) {
            case 'closed':
                // Diferenciar entre Resolved (ID 2) y Closed (ID 3)
